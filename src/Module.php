...
//    }
    ...
    {
        $url = $this->urlGenerator->generate('toolbar', [
            'tag' => $this->logTarget->tag,
            'id' => $this->id,
        ]);
    ...
    protected function checkAccess($action = null)
    {
        $allowed = false;
        // TODO fix that
        $ip = $_SERVER['REMOTE_ADDR'];//$this->app->getRequest()->getUserIP();
        ...
        if ($allowed === false) {
            if (!$this->disableIpRestrictionWarning) {
                $this->logger->warning('Access to debugger is denied due to IP address restriction. The requesting IP address is ' . $ip);
            }
        ...
        if ($this->checkAccessCallback !== null && call_user_func($this->checkAccessCallback, $action) !== true) {
            if (!$this->disableCallbackRestrictionWarning) {
                $this->logger->warning('Access to debugger is denied due to checkAccessCallback.');
            }
...
//    }
    ...
    public function htmlTitle()
    {
        ...
        if (is_callable($this->pageTitle)) {
            return call_user_func($this->pageTitle, $this->urlGenerator->generate('/'));
        }
